...
func NewGenetic(target float64, p *pgs.PGS) *Genetic {
	return &Genetic{
		target: target,
		p:      p,
	}
...
func (g *Genetic) Solve(numThreads int) map[string][]uint8 {
	...
	for k := 0; k < params.ITERATIONS; k++ {
		...
		candidates, deltas = g.mutate(candidates, deltas)
		candidates, deltas, solutions = g.checkForSolutions(candidates, deltas, solutions)
		children := g.crossover(candidates)
		chDeltas := g.calculateDeltas(children)
		children, chDeltas, solutions = g.checkForSolutions(children, chDeltas, solutions)
		candidates = g.tournament(append(candidates, children...), append(deltas, chDeltas...), poolSize)
	}
...
func (g *Genetic) crossover(population [][]uint8) [][]uint8 {
	...
	splice := func(first, second int) []uint8 {
		...
		likelihoods := make([]float64, len(parents[first])/pgs.NumHaplotypes)
		...
		fitness := FitnessFromLikelihoods(likelihoods)
...
func (g *Genetic) tournament(population [][]uint8, deltas []float64, populationSize int) [][]uint8 {
	//fitness := g.FitnessFromDeltasAndLikelihoods(population, deltas)
	fitness := FitnessFromDeltas(deltas)
...
func (g *Genetic) mutate(population [][]uint8, deltas []float64) ([][]uint8, []float64) {
	for j, original := range population {
		population[j], deltas[j] = g.p.MutateGenome(original, deltas[j])
	}
	return population, deltas
}

func FitnessFromDeltas(deltas []float64) []float64 {
...
func FitnessFromLikelihoods(likelihoods []float64) []float64 {
...
func (g *Genetic) FitnessFromDeltasAndLikelihoods(population [][]uint8, deltas []float64) []float64 {
	likelihoods := make([]float64, len(population))
	for i := 0; i < len(population); i++ {
		likelihoods[i] = g.p.CalculateSequenceLikelihood(population[i])
	}
	fitness := make([]float64, len(population))
	for i := range population {
		fitness[i] = 1 / (math.Abs(deltas[i]) * math.Abs(likelihoods[i]))
	}
	return fitness
}
